...
clientController.renderClients = async (request, response) => {
	const clients = await Client.getClientsWithGymName();
	sendClients(request, response, clients);
};
...
clientController.registerClient = async (request, response) => {
	...
	(nameTemporal = name),
		(lastNameTemporal = lastName),
		(phoneTemporal = phone),
		(gymTemporal = gym),
		(finalDateTemporal = finalDate),
		(monthsTemporal = months);
	...
	if (isEmpty(name, lastName, phone, gym, finalDate, months))
		...
		if (!clientFound) {
			...
			sendMessage(
				...
				response,
				'successMessage',
				`${name + ' ' + lastName} successfully added.`,
				'/clients'
			);
		} else
			sendMessage(
				...
				response,
				'errorPhone',
				`The client with phone ${phone} already exists`,
				'/client/add'
			);
	}
};
...
clientController.editClient = async (request, response) => {
	const { name, lastName, phone, gym, initialDate, finalDate, times } =
		request.body;
	...
	if (isEmpty(name, lastName, phone, gym, initialDate, finalDate, times))
		...
		sendMessage(
			...
			response,
			'errorDate',
			"The final date can't be greater than initial date",
			directionToBack
		);
		...
		if (!clientFound || clientFound.phone == phone) {
			...
			sendMessage(
				...
				response,
				'successMessage',
				`${name + ' ' + lastName} successfully updated`,
				'/clients'
			);
		} else
			sendMessage(
...
clientController.deleteClient = async (request, response) => {
	const clientToDelete = await Client.find(
	...
	sendMessage(
		...
		'successMessage',
		`${
			clientToDelete[0].name + ' ' + clientToDelete[0].lastName
		} delete successfully`,
		'/clients'
	);
};
...
clientController.searchClients = async (request, response) => {
	...
	if (Number.isInteger(parseInt(data)))
		...
		clients = await Client.find(
			{ name: data.toLowerCase().replace(/\s+/g, '') },
			{ phone: 0 }
		);
	...
	else sendClients(request, response, clients);
};
